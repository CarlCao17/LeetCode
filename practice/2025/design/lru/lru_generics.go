...
func (lru *LRU[K, V]) Clear() {
	if lru.onEvict != nil {
		lru.ForEach(func(node *Node[K, V]) bool {
...
func (l *list[K, V]) ForEach(fn func(node *Node[K, V]) bool) {
...
func (l *list[K, V]) ForEachReverse(fn func(node *Node[K, V]) bool) {
	for curr := l.tail.prev; curr != l.head; curr = curr.prev {
		if !fn(curr) {
			break
		}
	}
...
// node 指针由使用方来保证消除对现有的 prev/next 引用
