...
func Test_swapNodes(t *testing.T) {
	...
	cases := []testCase{
		...
		},
		{
			...
			expect: arrayToLinkList([]int{5, 9, 6, 0, 7, 8, 3, 6, 9, 7}),
		},
		{
			args: args{
				head: arrayToLinkList([]int{1}),
				k:    1,
			},
			expect: arrayToLinkList([]int{1}),
		},
		{
			args: args{
				head: arrayToLinkList([]int{1, 2}),
				k:    1,
			},
			expect: arrayToLinkList([]int{2, 1}),
		},
	}
