...
简单方案 (短暂停写)：如果业务允许，可以在预热开始前，短暂地禁止对数据库相关表的写操作，然后执行数据同步程序，完成后再放开写权限。这是最简单、最能保证初始一致性的方法。

复杂方案 (不停机同步)：对于不能停机的系统，可以采用以下步骤：

启动一个同步程序，全量将数据库中的数据读取并写入 Redis。

在同步期间，数据库的任何数据变更，除了写入数据库，还必须通过某种机制通知同步程序（例如，写入消息队列 MQ）。

全量同步完成后，程序开始消费 MQ 中的增量变更消息，追平在同步期间发生的数据变化。
...
预热阶段：通过“停写”或“CDC”等方式，保证 Redis 拥有一个准确的初始数据快照。
